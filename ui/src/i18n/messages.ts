...
export const en = {
  ...
  'ag.title': 'Agent CLI updates',
  'ag.hint': 'Claude Code and Codex each know how they were installed. Marol asks them to update themselves the first time it reaches a world — which for the worlds you use is when it opens.',
  'ag.onStart': 'Update the agent CLIs when Marol reaches a world',
  'ag.onStartHint': 'A new version installs beside the running one and is picked up by the next session you start, never by one already open. An update that fails leaves the CLI exactly where it was.',
  'ag.updated': '{agent} on {world}: {from} → {to}',
  'ag.current': '{agent} on {world}: already {to}',
  'ag.failed': '{agent} on {world}: could not update ({detail})',
  'ag.none': 'Nothing updated yet this run.',
...
export const zhTW: Record<MessageKey, string> = {
  ...
  'ag.title': 'Agent CLI 更新',
  'ag.hint': 'Claude Code 和 Codex 各自知道自己是怎麼裝的。Marol 第一次連到一個世界時會請它們自己更新 —— 對你在用的世界來說，那就是開啟的時候。',
  'ag.onStart': '連到一個世界時更新它的 agent CLI',
  'ag.onStartHint': '新版本會裝在執行中的那份旁邊，由你下一個開的 session 接手，不會動到已經開著的。更新失敗就是 CLI 留在原本的版本。',
  'ag.updated': '{world} 的 {agent}：{from} → {to}',
  'ag.current': '{world} 的 {agent}：已經是 {to}',
  'ag.failed': '{world} 的 {agent}：更新不了（{detail}）',
  'ag.none': '這次啟動還沒有更新過任何東西。',
